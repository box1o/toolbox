...
[[nodiscard]] constexpr BufferUsageFlags operator|(
    BufferUsageFlags a, BufferUsageFlags b) noexcept {
...
public:
    ...
    [[nodiscard]] virtual result<void> Update(
        const void* data, u64 size, u64 offset = 0) noexcept = 0;

    template <class T> [[nodiscard]] result<void> Update(const T& v, u64 offset = 0) noexcept {
    ...
    template <class T>
    [[nodiscard]] static result<ref<Buffer>> CreateUniform(
        ref<Device> device, const std::string& name = "UniformBuffer") noexcept {
        BufferDesc d{};
        d.size = (u64)sizeof(T);
